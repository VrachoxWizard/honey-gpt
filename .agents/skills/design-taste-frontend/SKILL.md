...
* **1-3 (Static):** No automatic animations. CSS `:hover` and `:active` transitions only.
* **4-7 (Dynamic):** Infrequent entry animations (e.g., mounting fade-in), slide-over panels, popover states.
* **8-10 (Cinematic):** Continuous interactive canvas patterns, complex SVG path animations, magnetic button groups, layouts shifting smoothly on scroll/pan.
