import { Download, Flame, KeyRound, MessageSquarePlus, Search, Sparkles, WandSparkles, X, MessageSquare, Trash2, Pencil, Check, ChevronDown } from 'lucide-react';
      ...
      <div className="flex flex-col h-full p-6 relative overflow-y-auto overflow-x-hidden">
        ...
        <div className="flex-1 overflow-y-auto mb-6 pr-1 space-y-2 select-none min-h-[120px] scrollbar-thin flex flex-col">
          <div className="flex items-center justify-between px-2 mb-2 shrink-0">
            <p className="text-[9px] font-bold tracking-widest text-zinc-500 uppercase">Povijest razgovora</p>
            {sessions.length > 1 && (
              <button 
        ...
        <div className="grid gap-2 mt-auto shrink-0 pt-4 border-t border-white/5">
          ...
          <div className="flex items-center gap-3 p-2.5 rounded-lg bg-zinc-900/30 border border-white/5 relative">
            ...
            <div className="flex flex-col flex-1 min-w-0 pr-6 relative">
              <label htmlFor="tone-select" className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider block">Stil satire</label>
              <select 
                id="tone-select"
                value={toneMode} 
              ...
              >
                <option value="sanctus" className="bg-zinc-950 text-zinc-300 text-xs py-1">† Sveti mod</option>
                <option value="clericus" className="bg-zinc-950 text-zinc-300 text-xs py-1">† Birokratski mod</option>
                <option value="humilis" className="bg-zinc-950 text-zinc-300 text-xs py-1">† Ponizni mod</option>
              </select>
          ...
          <div className="flex items-center gap-3 p-2.5 rounded-lg bg-zinc-900/30 border border-white/5 relative">
            ...
            <div className="flex flex-col flex-1 min-w-0 pr-6 relative">
              <label htmlFor="model-select" className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider block">Aktivni model</label>
              <select 
                id="model-select"
              ...
              >
                {!AVAILABLE_MODELS.some(m => m.id === activeModel) && activeModel && (
                  <option value={activeModel} className="bg-zinc-950 text-zinc-300 text-xs py-1">
                    {activeModel.replace(/^(google\/|qwen\/|meta-llama\/|deepseek\/|mistralai\/)/, '').split(':')[0]}
                  </option>
                )}
                {AVAILABLE_MODELS.map((m) => (
                  <option key={m.id} value={m.id} className="bg-zinc-950 text-zinc-300 text-xs py-1">
                    {m.name}
                  </option>
                ))}
              </select>
          ...
          <div className="flex items-center gap-3 p-2.5 rounded-lg bg-zinc-900/30 border border-white/5">
            ...
            <div className="flex flex-col flex-1">
              <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">Vjera u Boga</span>
          ...
          <div className="flex items-center gap-3 p-2.5 rounded-lg bg-zinc-900/30 border border-white/5">
            ...
            <div className="flex flex-col flex-1">
              <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">API ključ</span>
